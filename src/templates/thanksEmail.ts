...
export const newTahnksEmailTemplate = (
...
<body>
    <div class="email-container">
        ...
        <div class="order-details">
            ...
            <p><strong>Cliente:</strong> ${sale.profile?.name} ${
              sale.profile?.lastname
            }</p>
            <p><strong>Email:</strong> ${sale.profile?.email}</p>
            <p><strong>Telefono:</strong> ${sale.profile?.phone}</p>
            <p><strong>Estado del pago:</strong> ${
              paymentStateParser(sale?.status || "").text
            }</p>
        </div>
        ...
        <table>
            ...
            ${products.map((product: SaleItemsFullDetails) => {
              return `
                 <tr>
                <td>
                <img src="${product?.file?.image_url}" alt="${product?.file?.file_name}" style="width: 80; height: 80px; margin-bottom: 10px;"/>
                </td>
                <td>${product?.product?.name}</td>
                <td>${product?.file?.folder?.school?.name}</td>
                <td>${product?.file?.folder?.title}</td>
                <td>$${product.price}</td>
                <td>${product.quantity}</td>
                <td>$${product.total}</td>
            </tr>
